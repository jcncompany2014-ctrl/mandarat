...
- [x] 28. 저장 견고성: AppState 백그라운드 전환 시 디바운스 무시하고 즉시 플러시(마지막 변경 유실 방지). 표시기는 noise라 생략.
...
- [x] 35. 영역별 8각 레이더(LotusRadar) + 통계 히트맵에 "최고의 날 N회 · 날짜" 표기 (추세선은 추후)
...
- #4(만다라 휠) 영역 꽃잎·실천 노드·중앙 닫기·세부보기 탭에 접근성 라벨 추가 (전 화면 a11y 마감)
- 마이크로카피: Detail 영역 헤더가 연속 0일일 때 "오늘 시작해보세요"로 표시
- 코드품질/UX: 미사용 guideScore를 GuideChips에 활용 — 3요소 충족 시 골드 긍정 피드백 문구
- #28 저장 견고성: 앱 백그라운드 전환 시 AsyncStorage 즉시 플러시(디바운스 손실 방지)
- #25 강화: migrate가 themes 8슬롯·actions 8개를 슬롯별로 검증·보정 — 손상 저장본 크래시 방지
- #35 통계 히트맵 카드에 최근 70일 "최고의 날(횟수·날짜)" 표기 추가
- 신규: 전체 64칸 100% 도달 시 "만개(FULL BLOOM)" 특별 축하 — 일일 완료 축하와 카피 구분(celebrate kind: today|bloom)
- UX 일관성: 만다라 휠 중앙(핵심 목표) 탭 시 편집 열림(그리드 중앙과 동작 통일) + 접근성 라벨
- #35 주간 카드에 "하루 평균" 실천 횟수 표기 추가
- UI 버그(중대): 만다라 휠 중앙(핵심 목표) 노드가 점으로 collapse — Ring children 래퍼에 크기가 없어 absolute(left/right/top/bottom) 내부 원이 0px가 됨. 중앙·블룸 중앙을 명시 100×100으로 교체해 핵심 목표+%가 다시 보이도록 수정
- UI 다듬기: 대시보드 빈 상태 히어로 플레이스홀더가 "입력하/세요"로 단어 중간 줄바꿈 → "핵심 목표를 / 입력해 주세요" 어절 경계로 교정(빈 상태 전용 크기/톤)
- 전수 점검: 홈·만다라휠·격자81·상세·오늘·통계·설정 + 오버레이(셀편집·빠른추가·집중타이머·실천시트)·온보딩을 채움/빈 상태 모두 캡처 검증 — 위 2건 외 비율·정렬·간격 이상 없음 확인
